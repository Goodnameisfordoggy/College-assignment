 ...
 * @FilePath: \java BasicHomework\0503 The people of our provinces must carry out safe production in accordance with the instructions of the State Council.编写一个Java应用程序描述上述的体制现象。\test.java
 * @Description: 题目内容：
（1）定义一个“党中央”接口：CentralPartyCommittee， 该接口中有个“坚持党的领导”方法：void partyLeader()。
（2）定义一个“国务院”抽象类：StateCouncil， 该抽象类中有个“安全生产”的抽象方法： void safetyInProduction()。
（3）定义一个“省政府”类：Province， 该类有一个成员变量name，该类继承StateCouncil类并实现CentralPartyCommittee接口；
重写partyLeader()方法和safetyInProduction()方法 。定义show方法，输出省份名称。
